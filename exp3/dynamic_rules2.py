...
class dynamic_rules(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions, table_id=0, buffer_id=None, idle_timeout=0, hard_timeout=0):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def install_path(self, src_dpid, dst_dpid, src_port, dst_port, ev, src, dst, pkt_ipv4, pkt_tcp):
        ...
        self.logger.info("current_path : %s", str(self.current_path))
        self.logger.info("old_path : %s", str(self.old_path))
        ...
        for i in xrange(len(self.path) - 2, -1, -2):
            ...
            self.add_flow(datapath_path, 100, match, actions, table_id=0, idle_timeout=0, hard_timeout=0)
